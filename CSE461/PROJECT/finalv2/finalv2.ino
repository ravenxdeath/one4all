...
bool leftCommandReceived = false;
bool rightCommandReceived = false;
...
void loop() {
  ...
  if (carActivated && forwardCommandReceived && !stopCommandReceived) {
    checkAndMoveForward();
  } else if (carActivated && backwardCommandReceived && !stopCommandReceived) {
    checkAndMoveBackward();
  } else if (carActivated && leftCommandReceived && !stopCommandReceived) {
    checkAndTurnLeft();
  } else if (carActivated && rightCommandReceived && !stopCommandReceived) {
    checkAndTurnRight();
  } else {
    stopMotors();
  }
