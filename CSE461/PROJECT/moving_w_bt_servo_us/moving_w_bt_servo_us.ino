...
void checkAndMoveBackward() {
  if (!isObstacleDetected()) {
    moveBackward();
    delay(4000); // Move backward for 4 seconds
    stopMotors();
  }
}
...
void checkAndTurnLeft() {
  if (!isObstacleDetected()) {
    turnLeft();
    delay(4000); // Turn left for 4 seconds
    stopMotors();
  }
}
...
void checkAndTurnRight() {
  if (!isObstacleDetected()) {
    turnRight();
    delay(4000); // Turn right for 4 seconds
    stopMotors();
  }
}
...
bool isObstacleDetected() {
  ...
  return distance < 30; // Adjust threshold distance as needed
}
