...
lives = 3  # Spaceship starts with 3 lives
game_state = {
    'left_pressed': False,
    'right_pressed': False
}
...
def check_collision():
    ...
    # Check if an asteroid collides with the spaceship
    for obs in obstacles[:]:
        obs_x, obs_y, obs_size = obs
        if abs(spaceship_x - obs_x) < obs_size and abs(-230 - obs_y) < obs_size:
            lives -= 1  # Decrease lives
            obstacles.remove(obs)  # Remove the collided asteroid
            if lives == 0:
                game_over()
...
def display():
    ...
    # Print the current score in the console
